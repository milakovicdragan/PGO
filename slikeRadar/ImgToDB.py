...
class ConvertNameToDateTime:
    ...
    def rename_img_save_to_db(self):
        ...
        for i in range(len(paths)):
            ...
            if '_Base' in file_name:
                self.proccess_file(file_name, ext, pgo_folder_path, paths[i], '_Base', SlikeBase)
                print('Izvrsava _Base')
            if '_CAPPI' in file_name:
                self.proccess_file(file_name, ext, pgo_folder_path, paths[i], '_CAPPI', SlikeCappi)
                print('Izvrsava _CAPPI')
            if '_CMax' in file_name:
                #provjeravam da li iza _CMax rijeci ima jos rijeci ako ima onda je to _CMaxnoCor
                if re.search(r'_CMax\w+', file_name):
                    if re.search(r'_CMaxno\w+', file_name):
                        self.proccess_file(file_name, ext, pgo_folder_path, paths[i], '_CMaxnoCor', SlikeCMaxnoCor)
                        print('Izvrsava _CMaxnoCor')
                    if re.search(r'_CMaxZD\w+', file_name):
                        self.proccess_file(file_name, ext, pgo_folder_path, paths[i], '_CMaxZDR', SlikeCMaxZDR)
                        print('Izvrsava _CMaxZDR')
                    if re.search(r'_CMaxRain\w+', file_name):
                        self.proccess_file(file_name, ext, pgo_folder_path, paths[i], '_CMaxRainRate', SlikeCMaxRainRate)
                        print('Izvrsava _CMaxRainRate')
                else:
                    self.proccess_file(file_name, ext, pgo_folder_path, paths[i], '_CMax', Slike)
                    print('Izvrsava _CMax')
            if '_HMax' in file_name:
                self.proccess_file(file_name, ext, pgo_folder_path, paths[i], '_HMax', SlikeHmax)
                print('Izvrsava _HMax')
            if '_Nowcast' in file_name:
                self.proccess_file(file_name, ext, pgo_folder_path, paths[i], '_Nowcast', SlikeNowcast)
                print('Izvrsava _Nowcast')
            if '_VIL' in file_name:
                self.proccess_file(file_name, ext, pgo_folder_path, paths[i], '_VIL', SlikeVIL)
                print('Izvrsava _VIL')
            if '_PPI' in file_name:
                self.proccess_file(file_name, ext, pgo_folder_path, paths[i], '_PPI', SlikePPI)
                print('Izvrsava _PPI')

    def proccess_file(self, file_name, ext, pgo_folder_path, old_name_img, img_type, model):
        """pomocni metod koji img_type uzima fajl kako bi ga preimenovao i sacuvao u db model"""
        ...
        file_name = file_name.replace(img_type, '')
        #print(file_name)
        ...
        if os.path.exists(new_name_img):
            ...
            b = model(title=file_name, descr=date_time_desc, time_create=time_create)
            b.img.save(file_name + ext, pic)
            pic.close()
            # ukoliko je slika sacuvana u db onda cu je obrisati iz foldera Queue
            if model.objects.filter(descr=date_time_desc).exists():
    ...
    def proccess_file_CMaxnoCor(self, file_name, ext, pgo_folder_path, old_name_img):
        """pomocni metod koji _CMaxnoCor uzima fajl kako bi ga preimenovao i sacuvao u db SlikeCMaxnoCor"""
        # sa imena slike koji je u obliku '2023-01-10-0522_CMax' skidam naziv '_CMax' i dobijam sadrzaj varijable
        # u obliku '2023-01-10-0522'
        file_name = file_name.replace('_CMaxnoCor', '')
        # Radim rename slike iz formata '2023-01-10-0522_CMax.jpg' u '2023-01-10-0522.jpg' i smijestam
        # u varijablu pod imenom new_name_img, i ova se varijabla koristi kao putanja za uploda
        # slike u models
        new_name_img = os.path.join(pgo_folder_path, file_name + ext)
        ...
        if os.path.exists(new_name_img):
            ...
            # kreiram obj koji pokazuje na models.py i sluzi da sacuva sliku u bazu podataka
            pic = File(open(new_name_img, 'rb'))
            b = SlikeCMaxnoCor(title=file_name, descr=date_time_desc, time_create=time_create)
            b.img.save(file_name + ext, pic)
            pic.close()
            # ukoliko je slika sacuvana u db onda cu je obrisati iz foldera Queue
            if SlikeCMaxnoCor.objects.filter(descr=date_time_desc).exists():
                # obrisati sliku iz Queue foldera
                os.remove(new_name_img)
        else:
            raise Exception("Sorry, but file {file} does not exists".format(file=new_name_img))

    def proccess_file_Base(self, file_name, ext, pgo_folder_path, old_name_img):
        """pomocni metod koji _Base uzima fajl kako bi ga preimenovao i sacuvao u db SlikeBase"""
        # sa imena slike koji je u obliku '2023-01-10-0522_CMax' skidam naziv '_CMax' i dobijam sadrzaj varijable
        # u obliku '2023-01-10-0522'
        file_name = file_name.replace('_Base', '')
        # Radim rename slike iz formata '2023-01-10-0522_CMax.jpg' u '2023-01-10-0522.jpg' i smijestam
        # u varijablu pod imenom new_name_img, i ova se varijabla koristi kao putanja za uploda
        # slike u models
        new_name_img = os.path.join(pgo_folder_path, file_name + ext)
        ...
        if os.path.exists(new_name_img):
            ...
            # kreiram obj koji pokazuje na models.py i sluzi da sacuva sliku u bazu podataka
            pic = File(open(new_name_img, 'rb'))
            b = SlikeBase(title=file_name, descr=date_time_desc, time_create=time_create)
            b.img.save(file_name + ext, pic)
            pic.close()
            # ukoliko je slika sacuvana u db onda cu je obrisati iz foldera Queue
            if SlikeBase.objects.filter(descr=date_time_desc).exists():
                # obrisati sliku iz Queue foldera
                os.remove(new_name_img)
        else:
            raise Exception("Sorry, but file {file} does not exists".format(file=new_name_img))

    def proccess_file_Nowcast(self, file_name, ext, pgo_folder_path, old_name_img):
        """pomocni metod koji _Nowcast uzima fajl kako bi ga preimenovao i sacuvao u db SlikeNowcast"""
        # sa imena slike koji je u obliku '2023-01-10-0522_CMax' skidam naziv '_CMax' i dobijam sadrzaj varijable
        # u obliku '2023-01-10-0522'
        file_name = file_name.replace('_Nowcast', '')
        # Radim rename slike iz formata '2023-01-10-0522_CMax.jpg' u '2023-01-10-0522.jpg' i smijestam
        # u varijablu pod imenom new_name_img, i ova se varijabla koristi kao putanja za uploda
        # slike u models
        new_name_img = os.path.join(pgo_folder_path, file_name + ext)
        ...
        if os.path.exists(new_name_img):
            ...
            # kreiram obj koji pokazuje na models.py i sluzi da sacuva sliku u bazu podataka
            pic = File(open(new_name_img, 'rb'))
            b = SlikeNowcast(title=file_name, descr=date_time_desc, time_create=time_create)
            b.img.save(file_name + ext, pic)
            pic.close()
            # ukoliko je slika sacuvana u db onda cu je obrisati iz foldera Queue
            if SlikeNowcast.objects.filter(descr=date_time_desc).exists():
                # obrisati sliku iz Queue foldera
                os.remove(new_name_img)
        else:
            raise Exception("Sorry, but file {file} does not exists".format(file=new_name_img))

    def proccess_file_Hmax(self, file_name, ext, pgo_folder_path, old_name_img):
        """pomocni metod koji _HMax uzima fajl kako bi ga preimenovao i sacuvao u db SlikeHmax"""
        # sa imena slike koji je u obliku '2023-01-10-0522_CMax' skidam naziv '_CMax' i dobijam sadrzaj varijable
        # u obliku '2023-01-10-0522'
        file_name = file_name.replace('_HMax', '')
        # Radim rename slike iz formata '2023-01-10-0522_CMax.jpg' u '2023-01-10-0522.jpg' i smijestam
        # u varijablu pod imenom new_name_img, i ova se varijabla koristi kao putanja za uploda
        # slike u models
        new_name_img = os.path.join(pgo_folder_path, file_name + ext)
        ...
        if os.path.exists(new_name_img):
            ...
            # kreiram obj koji pokazuje na models.py i sluzi da sacuva sliku u bazu podataka
            pic = File(open(new_name_img, 'rb'))
            b = SlikeHmax(title=file_name, descr=date_time_desc, time_create=time_create)
            b.img.save(file_name + ext, pic)
            pic.close()
            # ukoliko je slika sacuvana u db onda cu je obrisati iz foldera Queue
            if SlikeHmax.objects.filter(descr=date_time_desc).exists():
                # obrisati sliku iz Queue foldera
                os.remove(new_name_img)
        else:
            raise Exception("Sorry, but file {file} does not exists".format(file=new_name_img))

    def proccess_file_CAPPI(self, file_name, ext, pgo_folder_path, old_name_img):
        """pomocni metod koji _CAPPI uzima fajl kako bi ga preimenovao i sacuvao u db SlikeCappi"""
        # sa imena slike koji je u obliku '2023-01-10-0522_CMax' skidam naziv '_CMax' i dobijam sadrzaj varijable
        # u obliku '2023-01-10-0522'
        file_name = file_name.replace('_CAPPI', '')
        # Radim rename slike iz formata '2023-01-10-0522_CMax.jpg' u '2023-01-10-0522.jpg' i smijestam
        # u varijablu pod imenom new_name_img, i ova se varijabla koristi kao putanja za uploda
        # slike u models
        new_name_img = os.path.join(pgo_folder_path, file_name + ext)
        ...
        if os.path.exists(new_name_img):
            ...
            # kreiram obj koji pokazuje na models.py i sluzi da sacuva sliku u bazu podataka
            pic = File(open(new_name_img, 'rb'))
            b = SlikeCappi(title=file_name, descr=date_time_desc, time_create=time_create)
            b.img.save(file_name + ext, pic)
            pic.close()
            # ukoliko je slika sacuvana u db onda cu je obrisati iz foldera Queue
            if SlikeCappi.objects.filter(descr=date_time_desc).exists():
                # obrisati sliku iz Queue foldera
                os.remove(new_name_img)
        else:
            raise Exception("Sorry, but file {file} does not exists".format(file=new_name_img))

    def proccess_file_Cmax(self, file_name, ext, pgo_folder_path, old_name_img):
        """pomocni metod koji _CMax uzima fajl kako bi ga preimenovao i sacuvao u db Slike"""
    ...
    def time_query_iteration(self, model, time_inc):
        """definisem metod koji koristi query_to_create_gif()
         Ovom funkcijom  cu kroz petlju napraviti queri sa svim slikama u vremenskom rayamaku od
        npr 15min. tj svaku 15min sliku ce samo iydvojiti iy baze. Slike ce se izdovijiti od najstarije
        fisrt_query do najnovije slike first_query.
        ...
        :return vraza queri sa slikama u vremenskom razmaku definisan sa time_inc
        """
        ...
        delta = timedelta(minutes=time_inc)
        while start_time <= end_time:
            title_value = start_time.strftime("%Y-%m-%d-%H%M")
            # print(title_value)
            if model.objects.filter(title__icontains=title_value):
                for i in model.objects.filter(title__icontains=title_value):
                    # query = list(chain(model.objects.filter(title__icontains=title_value)))
                    var_for_loop.append(i.title)
            start_time += delta
        print(var_for_loop)
        print(model.objects.filter(title__in=var_for_loop))
        return model.objects.filter(title__in=var_for_loop)

    def query_to_create_gif(self, GIFmodel, model, time_inc):
        '''
        ...
        query_descr = self.time_query_iteration(model, time_inc)
        gif_title = ''
        gif_img_name = ''
        ...
        if os.path.exists(path_to_gif_img):
            ...
            b = GIFmodel(title=gif_title, descr=gif_img_name)
            ...
            if GIFmodel.objects.filter(descr=gif_img_name).exists():
